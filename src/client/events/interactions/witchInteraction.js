...
class witchInteraction {
  isButtonPoison = async (interaction, gameRoom) => {
    ...
    try {
      const witch = gameRoom.players.find((p) => p.role.id === WEREROLE.WITCH);
  ...
  isButtonHeal = async (interaction, gameRoom) => {
    const playerId = interaction.customId.split('_')[3];
  ...
  isModalSubmitPoison = async (interaction, gameRoom, sender, client) => {
  ...
  isModalSubmitHeal = async (interaction, gameRoom, sender, client) => {
    if (!gameRoom || gameRoom.gameState.phase !== 'night') return;
...
module.exports = new witchInteraction();
