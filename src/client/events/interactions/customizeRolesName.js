...
class CustomizeRolesName {
  ...
  isModalSubmit = async (interaction, gameRooms) => {
    ...
    const roleNameMap = {
      'masoi': 0, 'werewolf': 0, 'ma sói': 0,
      'danlang': 1, 'villager': 1, 'dân làng': 1,
      'baove': 2, 'bodyguard': 2, 'bảo vệ': 2,
      'bansoi': 3, 'cursed': 3, 'bán sói': 3,
      'tientri': 4, 'seer': 4, 'tiên tri': 4,
      'thamtu': 5, 'detective': 5, 'thám tử': 5,
      'phuthuy': 6, 'witch': 6, 'phù thủy': 6,
      'thangngo': 7, 'fool': 7, 'thằng ngố': 7,
      'thaydong': 8, 'medium': 8, 'thầy đồng': 8,
      'haugai': 10, 'maid': 10, 'hầu gái': 10,
      'lycan': 11,
      'soitientri': 12, 'wolfseer': 12, 'sói tiên tri': 12,
      'soitrum': 13, 'alphawerewolf': 13, 'sói trùm': 13,
      'cao': 14, 'foxspirit': 14, 'cáo': 14,
      'gialang': 15, 'elder': 15, 'già làng': 15,
      'stalker': 16, 'hori': 16, 'stakẻ': 16,
      'xathu': 17, 'gunner': 17, 'xạ thủ': 17,
      'soimeocon': 18, 'kittenwolf': 18, 'sói mèo con': 18
    };
    ...
    try {
      ...
      for (const pair of pairs) {
        ...
        if (!roleName || !countStr) {
          throw new Error(`Định dạng không hợp lệ: "${pair}". Sử dụng định dạng "tên_vai_trò: số_lượng"`);
        }

        const roleId = roleNameMap[roleName.toLowerCase()];
        if (roleId === undefined) {
          throw new Error(`Tên vai trò không hợp lệ: "${roleName}". Xem tên từng role, vd: Thầy Đồng -> thaydong hoặc medium`);
        }

        const count = parseInt(countStr);
        if (isNaN(count) || count <= 0) {
          throw new Error(`Số lượng không hợp lệ cho vai trò "${roleName}": ${countStr}`);
        }
      ...
      return interaction.editReply({
        content: `Lỗi phân tích cú pháp: ${err.message}`,
        ephemeral: true,
      });
    }
...
module.exports = new CustomizeRolesName(); 